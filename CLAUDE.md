...
| `GEMINI_API_KEY` | Yes, for generation/rewrite to work | `lib/ai/gemini.ts` | Missing key → `503 AIServiceError`, not a startup crash — the app boots fine without it, only AI calls fail. |
| `GEMINI_MODEL` | No | `lib/ai/gemini.ts` | Defaults to `"gemini-2.5-flash"` if unset. |
